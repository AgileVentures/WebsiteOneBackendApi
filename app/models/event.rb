...
class Event < ApplicationRecord
  ...
  validates :repeats_every_n_weeks, presence: true, if: ->(e) { e.repeats == 'weekly' or e.repeats == 'biweekly' }
  validates :repeat_ends_on, presence: true, allow_blank: false,
    if: ->(e){ (e.repeats == 'weekly' or e.repeats == 'biweekly') and e.repeat_ends_string == 'on' }
  validate :must_have_at_least_one_repeats_weekly_each_days_of_the_week,
    if: ->(e) { e.repeats == 'weekly' or e.repeats == 'biweekly' }
  ...
  def self.future_events
    Event.where('start_datetime > ? OR repeat_ends = false AND repeats != ? OR repeat_ends = false ' \
      ' AND repeat_ends_on IS NULL OR repeat_ends_on > ?', 1.day.ago, 'never', 1.day.ago)
  ...
  def self.upcoming_events(project=nil)
    events = Event.base_future_events(project).inject([]) { |memo, event| memo << event.next_occurrences }
     .flatten.sort_by { |e| e[:time] }
  ...
  def series_end_time
    repeat_ends && repeat_ends_on.present? ? repeat_ends_on.in_time_zone : nil
  end
  ...
  def next_occurrence_time_method(start=Time.zone.now)
  ...
  def self.next_occurrence(event_type, begin_time=NEXT_SCRUM_COLLECTION_TIME_PAST.ago)
    events_with_times = Event.where(category: event_type).map do |event|
  ...
  # To cover these cases, the while loop looks farther and farther into the future for the next event occurrence,
  # just in case there are many exclusions.
  def next_event_occurrence_with_time(start=Time.zone.now, final=2.months.from_now)
  ...
  def next_occurrences(options={})
    ...
    [].tap do |occurences|
      occurrences_between(begin_datetime, final_datetime).each do |time|
        occurences << ({ event: self, time: })
        return occurences if occurences.count >= limit
      end
    end
  end

  # rubocop wants: delegate :occurrences_between, to: :schedule
  ...
  def repeats_weekly_each_days_of_the_week=(repeats_weekly_each_days_of_the_week)
    self.repeats_weekly_each_days_of_the_week_mask = (repeats_weekly_each_days_of_the_week & DAYS_OF_THE_WEEK)
      .map { |r| 2**DAYS_OF_THE_WEEK.index(r) }.inject(0, :+)
  end
  ...
  def remove_from_schedule(timedate)
    ...
    if timedate >= Time.zone.now && timedate == next_occurrence_time_method
  ...
  def schedule
    if series_end_time.nil? || !repeat_ends
      sched = IceCube::Schedule.new(start_datetime)
    else
      sched = IceCube::Schedule.new(start_datetime, end_time: series_end_time)
    end
    case repeats
    when 'never'
  ...
  def less_than_ten_till_start?
    ...
    Time.zone.now > next_event_occurrence_with_time[:time] - 10.minutes
  rescue
    false
  end
